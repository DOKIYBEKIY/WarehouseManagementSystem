...
<div class="card">
<h2>出库记录查询</h2>
<form method="get" action="${pageContext.request.contextPath}/query/outStock">
...
<tr><th>编号</th><th>商品名称</th><th>仓库</th><th>货主</th><th>数量</th><th>单价</th><th>总金额</th><th>操作员</th><th>出库时间</th></tr>
<c:forEach items="${outList}" var="s">
<tr><td>${s.outId}</td><td>${s.goodsName}</td><td>${s.warehouseName}</td><td>${s.ownerName}</td><td>${s.quantity}</td><td>${s.unitPrice}</td><td>${s.totalAmount}</td><td>${s.operatorName}</td><td>${s.outTime}</td></tr>
</c:forEach>
<c:if test="${empty outList}"><tr><td colspan="9" style="text-align:center;">暂无数据</td></tr></c:if>
</table></div>
</div>
